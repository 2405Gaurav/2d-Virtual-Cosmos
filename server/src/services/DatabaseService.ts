...
export class DatabaseService {
  async upsertUser(username: string): Promise<{ color: string; lastX: number; lastY: number }> {
    try {
      ...
      return { color: user.color, lastX: user.lastX, lastY: user.lastY }
    } catch {
      return { color: COLORS[0], lastX: 400, lastY: 300 }
    }
  }
